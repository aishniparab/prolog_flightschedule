%Flight path program file - functions.pl
%cmps112 asgn4
%written by:
% aparab@ucsc.edu - Aishni Parab
% ehobbs@ucsc.edu - Evan Hobbs
%
...
arrival_time( Start, Dest, DTime, ATime) :-
   ...
   Temp1 is (Dist/500),
   Temp2 is Temp1 + DTime,
   ATime is (Dist/500.0) + DTime. 
...
findpath( From, To, BeenTo, [[From, Dtime, Atime] | List], DtimeHM) :-
   ...
   not( member( X, BeenTo)),
   hoursmins_to_hours( DtimeHM, Dtime),
   arrival_time( From, X, Dtime, Atime),
   Atime < 24.0,
   flight( X, _, XDtimeHM),
...
%print %two %numbers %when %printing %time
print_nums( Nums ) :-
...
writepath( [] ) :-
   nl.
%format the writing so that it looks nice.
%this is the last call to writepath, when it only has 2 more to write
...
%Main %function for finding flights
fly( From, From ) :-
...
fly( From, To ) :-
    ...
    !, fail.
fly( _, _) :-
    write( 'Airports given do not match those in our database.' ), nl,
    !, fail. 
